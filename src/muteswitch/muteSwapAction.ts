...
const muteSwapAction: ActivateZkAction = async ({ wallet }) => {
...
export const postMuteSwapAction: PostZkAction = async ({ wallet, recipient, minion }) => {
  console.log('ℹ️', 'POST SWAP ACTION...');
...
export const activateMuteSwapAccounts = async () => {
  await activateZkAccounts([muteSwapAction], {
    skipPostAction: true,
    postAction: postMuteSwapAction,
    L2ToL2Action: true
  });
};
